/*  객체는 프로퍼티의 집합니다.
    ...
    /* 프로퍼티 key 란?
        ...
        value 에 접근하기 위한 식별자의 역활을 하게 된다.
        문자열이기 때문에 key 는 '' 로 감싸야하지만,
        네이밍 규칙을 맞춘다면 '' 는 생략이 가능하다.
        규칙을 위반하게 되면 '' 반드시 사용해야 한다. */
    var object = {
        normal : 'hi', // 작성규칙 맞추면 '' 생략 가능
        '@ s p a c e' : 'space value', // 특수기호 규칙이 위반되게 때문에 ''를 꼭 감싸주어야 한다.
        '' : '', // 빈 문자열 경우에는 오류 발생 x 하지만 권장 하지 않음
        0 : 1, // 숫자 키는 내부적으로 문자열로 변환 된다.
        if : 'var', // 예약어를 key 로 사용할 수 있다. 하지만 권장 하지 않음
        normal : 'new value' // 이미 존재하는 키를 중복 선언하면 나중에 선언한 프로퍼티로 덮어 씌우게 된다.
    };
